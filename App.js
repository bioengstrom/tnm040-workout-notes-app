...
export default class App extends React.Component {
	constructor(props) {
		...
		this.state = {
			...
			relativeWeek: 0,
			relativeMonth: 0,
			plannedWorkoutArray: Array(7).fill(0)};
		...
		this.getRelativeMonth = this.getRelativeMonth.bind(this);
	...
	//Gets the relative week (according to the startup week).
	getRelativeMonth = (relation) => {
		//this.setPlannedWorkout(relation);
		this.setState({relativeMonth: relation});
		this.getRelativeWeek(moment().add(relation, 'month').endOf('isoweek').week());
		// console.log(relation.toString() + " " + (moment().add(relation, 'month').endOf('isoweek').week()).toString() + " " + moment().add(relation, 'month').endOf('isoweek').format('MMM'));
	}
	...
	highlightToday = () => {
		if(this.state.key==moment().format('L')) return true;
		return false;
	}
      ...
      <View style={styles.container}>
				<TouchableWithoutFeedback onPress={Keyboard.dismiss}>
    				...
    				<TouchableOpacity onPress={() => this.returnHome()} style={styles.logoStyle}>
    			 		<Image style={styles.logo} source={require("./images/logoBlue.png")}/>
    				</TouchableOpacity>
					</View>
				</TouchableWithoutFeedback>
				<CalendarMonth relativeWeek={this.props.relativeWeek} getRelativeMonth={this.getRelativeMonth} relativeMonth={this.state.relativeMonth}/>
      	...
      	<KeyboardAvoidingView style={styles.noteStyle} behavior={'padding'}>
					<View style={styles.textField}>
						<Text style={[styles.dateStyle, this.highlightToday()?[styles.today]:[styles.notToday]]}>
